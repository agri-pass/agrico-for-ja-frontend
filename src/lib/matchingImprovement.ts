...
import { OwnedFarmlandCSV, normalizeText, normalizeChiban } from './dataMatching';
...
export function improvedMatchFarmland(
  ...
  // 1. 地番マッチング
  const csvChiban = normalizeChiban(csvData.chiban + (csvData.edaban ? `-${csvData.edaban}` : ''));
...
export function improvedBatchMatching(
  ...
  for (const csv of csvData) {
    let bestMatch: { feature: FarmlandFeature; score: number; reasons: string[] } | null = null;
...
// マッチング結果の詳細分析
export function analyzeMatchingResults(results: any[]) {
