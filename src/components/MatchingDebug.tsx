...
import { performUnifiedMatching, UnifiedMatchingResult } from '@/lib/matchingService';
...
export default function MatchingDebug() {
  ...
  const [matchingResult, setMatchingResult] = useState<UnifiedMatchingResult | null>(null);
  const [loading, setLoading] = useState(false);
  const [selectedUnmatchedCSV, setSelectedUnmatchedCSV] = useState<OwnedFarmlandCSV | null>(null);
  const [selectedUnmatchedGeo, setSelectedUnmatchedGeo] = useState<FarmlandFeature | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setFarmlands(farmlandsData.map(f => ({ 
        ...f, 
      ...
      console.log(`Loaded ${farmlandsData.length} farmlands and ${parsedCSV.length} CSV records`);
    } catch (error) {
  ...
  const runMatching = () => {
    ...
    console.log(`Unique CSV matches: ${result.statistics.uniqueCSVMatches}/${result.statistics.totalCSV}`);
  };
    ...
    return (
      <div className="p-3 border rounded bg-gray-50">
        ...
        <div className="text-sm text-gray-600 mt-1">
          ...
          <div>地番: {csv.chiban}{csv.edaban ? `-${csv.edaban}` : ''}</div>
          <div className="mt-2 text-xs">
            <div>正規化後:</div>
            <div>大字: {normalizeText(csv.oaza)}</div>
            <div>小字: {normalizeText(csv.koaza)}</div>
            <div>地番: {normalizeChiban(csv.chiban + (csv.edaban ? `-${csv.edaban}` : '')).full}</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">マッチングデバッグツール</h1>
        ...
        <div className="flex gap-4 items-center">
          ...
          {matchingResult && (
            <div className="text-sm text-gray-600">
              マッチング率: <span className="font-bold text-green-600">
                {matchingResult.statistics.matchRate.toFixed(1)}%
              </span>
              ({matchingResult.statistics.matchedCount}/{matchingResult.statistics.totalCSV})
            </div>
          )}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            {selectedUnmatchedCSV ? (
                ...
                {farmlands
                  ...
                  .map((geo, index) => (
                    <div
                      key={index}
                      className={`cursor-pointer transition-colors ${
                        selectedUnmatchedGeo === geo ? 'ring-2 ring-blue-500' : ''
                      }`}
              ...
              <p className="text-gray-500">左側の未マッチCSVデータを選択してください</p>
            )}
          </div>
        </div>
      )}
      ...
      {selectedUnmatchedCSV && selectedUnmatchedGeo && (
        <div className="bg-yellow-50 p-4 rounded-lg shadow mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <h4 className="font-semibold text-blue-600 mb-2">GeoJSON データ</h4>
          ...
          <div className="mt-4 p-3 bg-white rounded border">
            ...
            <div className="text-sm space-y-1">
              ...
              <div>大字マッチ: {
                normalizeText(selectedUnmatchedGeo.properties.Address)
