import { FarmlandFeature, FarmerColorMap } from '@/types';

export function generateFarmerColors(features: FarmlandFeature[]): FarmerColorMap {
  const farmerHashes = [...new Set(features.map(f => f.properties.FarmerIndicationNumberHash))];
  
  const colors = [
    '#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A', '#98D8C8',
    '#F06292', '#AED581', '#FFD54F', '#4DD0E1', '#BA68C8',
    '#FF8A65', '#81C784', '#DCE775', '#4FC3F7', '#9575CD',
    '#FF9800', '#66BB6A', '#FFEB3B', '#42A5F5', '#7E57C2'
  ];
  
  const colorMap: FarmerColorMap = {};
  
  farmerHashes.forEach((hash, index) => {
    const farmlandsCount = features.filter(f => 
      f.properties.FarmerIndicationNumberHash === hash
    ).length;
